...
- **`agent/subagents/`** — each subagent is a `Harness` with its own system prompt and a restricted `tool_map`, wrapped by `Subagent` (`base.py`) which runs a task in a fresh conversation and records the result into `TaskState`. `explorer.py` (`build_explorer(client, policies, memory_tools)`) is a read-only subagent (`read_file`, `list_files`) that also reads/writes project memory (`read_memory`, `remember`) so it reuses and records what it learns about structure/deps/conventions; its schema is derived from its actual `tool_map`. `researcher.py` (`build_researcher(client, retrieve, web_search, policies)`) is a **RAG-first** subagent: its evidence tools are `retrieve` + `web_search` (RAG first, web fallback), plus a private structured-output tool `submit_research_result` that records `respuesta` and `fuentes` as JSON instead of a fragile text footer. `extract_sources` reads that structured payload so the orchestrator can `record_source`. With an empty/unavailable RAG index and no `TAVILY_API_KEY` both evidence tools degrade gracefully and the orchestrator records the missing external evidence via `record_missing_evidence`.
- **`agent/orchestrator.py`** — `Orchestrator` (the main agent): creates the `TaskState`, delegates to subagents as if they were tools, and renders the report (including a "Falta de evidencia" section from `state.missing_evidence`). Pipeline is a sequence of steps (`_explore` → `_research`); each records its result and sources into `TaskState`. New subagents plug into `run` as another step without changing the shape.
