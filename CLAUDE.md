...
- **`agent/factory.py`** — the only place tool name → function mapping is defined (`tool_map`); add new tools here plus in `tools.py` and the `TOOL_SCHEMAS` schema in `llm.py`.
