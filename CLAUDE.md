...
Env vars (`.env` or environment): `OPENAI_API_KEY` (required), `TAVILY_API_KEY` (optional — without it `web_search` returns an "unavailable" stub), `OPENAI_MODEL` (default `gpt-4o`), `AGENT_WORKSPACE` (default `./workspace`, where repos are cloned). Observability (all optional — without them Langfuse instrumentation is a no-op): `LANGFUSE_PUBLIC_KEY`, `LANGFUSE_SECRET_KEY`, `LANGFUSE_HOST`.
