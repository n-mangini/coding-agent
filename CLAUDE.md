...
Env vars (`.env` or environment): `OPENAI_API_KEY` (required), `TAVILY_API_KEY` (optional — without it `web_search` returns an "unavailable" stub), `OPENAI_MODEL` (default `gpt-4o`), `AGENT_WORKSPACE` (default `./workspace`, where repos are cloned), `RAG_PERSIST_DIR` (default `./rag_store`, where the Chroma index persists). Observability (all optional — without them Langfuse instrumentation is a no-op): `LANGFUSE_PUBLIC_KEY`, `LANGFUSE_SECRET_KEY`, `LANGFUSE_HOST`.
...
- **`agent/tools.py`** — the 6 tool implementations: `read_file`, `list_files`, `write_file`, `execute_command`, `web_search`, `retrieve`. Each returns a string (success text or an `Error: ...` message — tools never raise; errors go back to the LLM as content). `web_search` is built via `make_web_search(tavily_api_key)` and `retrieve` via `make_retrieve(rag_store)` so both can degrade to a stub.
...
- **`agent/harness.py`** — `Harness.run_conversation()` is the core loop: call LLM → if it returned `tool_calls`, execute each and append `role:"tool"` results, repeat; else return the final text. Each `Harness` carries its own `system_message` (default `SYSTEM_MESSAGE`), so subagents specialize by reusing the same engine. Two safeguards wrap the loop (TP Final, context management): **context summarization** (`_manage_context`/`_summarize` — when the history passes `HISTORY_LIMIT`, the old turns are condensed into one summary via a no-tools `call_llm` instead of resending everything; `_safe_tail_boundary` keeps the preserved tail from starting on an orphan `role:"tool"` message) and **loop detection** (`detect_loop` — a pure function flagging `LOOP_THRESHOLD` identical consecutive tool-call signatures; `_intervene_on_loop` escalates replan → stop: first it injects a replan nudge and resets the trace, then if it keeps looping it stops and asks for help; cada intervención queda en `loop_events` para que `Subagent.run` la registre en `TaskState.observations`). Two extra modes also live here: **Plan Mode** (`plan_mode_turn`/`generate_plan` — a separate no-tools LLM call that proposes a numbered plan and iterates on user feedback before execution) and **Supervision** (`_confirm_action` — human-in-the-loop confirmation, gated on the `approval` list from `agent.config.yaml`).
...
- **`agent/state.py`** — `TaskState`, the state shared across the orchestrator and every subagent (request, progress, per-subagent results, sources, modified files, observations, and `missing_evidence`). `missing_evidence` (with `record_missing_evidence`) is kept separate from `observations` so the report can be explicit about what could not be verified — the "reconocer falta de evidencia" requirement.
- **`agent/memory.py`** — per-project persistent memory as JSON (`.agent_memory.json` in the cwd, i.e. the cloned repo root; git-ignored). `ProjectMemory` holds seven categories (arquitectura, dependencias, comandos, convenciones, decisiones, bugs, resumenes — the `CATEGORIES` constant, source of truth for the `remember` schema too); `load_project_memory()` reads it (empty & graceful on missing/corrupt file), `remember` dedups + saves on each write, `render()` formats it as LLM context. `make_memory_tools(memory)` builds the `read_memory` / `remember` tools closed over one memory instance (same pattern as `make_web_search`), so agent and subagents share the project's memory.
- **`agent/subagents/`** — each subagent is a `Harness` with its own system prompt and a restricted `tool_map`, wrapped by `Subagent` (`base.py`) which runs a task in a fresh conversation and records the result into `TaskState`. `explorer.py` (`build_explorer(client, policies, memory_tools)`) is read-only plus project memory (`read_file`, `list_files`, `read_memory`, `remember`). `researcher.py` (`build_researcher(client, retrieve, web_search, policies)`) is RAG-first: evidence tools `retrieve` + `web_search`, plus private structured-output tool `submit_research_result`; `extract_sources` reads its JSON sources. `implementer.py` (`build_implementer`) writes the final report from `TaskState` and exposes a path-scoped `write_file` wrapper that only allows `REPORT_FILENAME` (`REPORTE-ANALISIS.md`). `tester.py` (`build_tester`) exposes `execute_command` but wraps it with an allowlist (`DEFAULT_TEST_COMMAND`, currently `python3 -m compileall ...`) so it runs one real check without becoming a general shell; failed checks become observations instead of breaking the pipeline. `reviewer.py` (`build_reviewer`) is read-only (`read_file`, `list_files`) plus private structured-output tool `submit_review_result`; `extract_observations` reads its JSON observations so the orchestrator can `record_observation`. Together they close the generate→validate→review loop.
- **`agent/orchestrator.py`** — `Orchestrator` (the main agent): creates the `TaskState`, delegates to subagents as if they were tools, and renders the report (including sources, Tester output, review observations, and "Falta de evidencia" from `state.missing_evidence`). Pipeline is a sequence of steps (`_explore` → `_research` → `_implement` → `_test` → `_review`); each records its result, sources, modified files, and observations into `TaskState`. New subagents plug into `run` as another step without changing the shape.
- **`agent/observability.py`** — Langfuse instrumentation, kept thin and non-invasive. `build_openai_client()` returns the OpenAI client wrapped with Langfuse's drop-in (`langfuse.openai.OpenAI`) when credentials are present, so every `call_llm` turn is traced as a generation (prompts, model, tokens, cost, latency, errors) **without adding a second OpenAI call site** — `llm.build_client()` delegates here. `trace_tool()` wraps each tool execution as a span (name, args, output, latency, errors); `Harness._execute_tool_call` calls it. `observed_run()` opens the use-case root trace (used by `orchestrator.run` and each chat turn in `main.py`) so all generations/spans nest into one trace. **Graceful degradation:** with no/invalid Langfuse credentials (or the lib missing), every helper is a no-op and the agent runs exactly as before — the instrumentation never crashes a run.
...
- **`rag/`** (project root, not part of the `agent/` package) — the RAG pipeline the `retrieve` tool sits on: `chunking.py` (`chunk_text`, char windows with overlap), `embeddings.py` (`embed_texts` with OpenAI `text-embedding-3-small`, reusing the shared/instrumented client), `store.py` (`RagStore` over a persistent Chroma collection + `make_rag_store`, which returns `None` when `chromadb` is missing so `retrieve` degrades to a stub), and `ingest.py` (`ingest_path` + a `python -m rag.ingest <path>` CLI to populate the index). It lives outside `agent/` (like `repo.py`) because it's a retrieval capability the agent *uses* via the tool, not part of the `Harness` loop. Persist dir: `RAG_PERSIST_DIR` (default `./rag_store`). Ingestion is an explicit step — run the CLI before expecting `retrieve` to return hits.
...
- Adding a tool requires three coordinated changes: the function in `tools.py`, its entry in `TOOL_SCHEMAS` (schema) in `llm.py`, and its entry in `tool_map` in `factory.py`. (Exception: the memory tools `read_memory`/`remember` live in `memory.py` because they must close over a `ProjectMemory` instance — like `web_search` closes over Tavily — so they're built by `make_memory_tools` rather than defined in `tools.py`.)
